...
namespace IBLT
{
    ...
    [Serializable]
    public class InvertibleBloomLookupTableXOR_ExtraDeletes
    {
        [JsonProperty]
        private int k;
        ...
        public InvertibleBloomLookupTableXOR_ExtraDeletes(int hashFunctions, int m)
        {
        ...
        public void Substract(InvertibleBloomLookupTableXOR_ExtraDeletes other)
        {
        ...
        public (int, byte[])[] GetAllItems()
        {
            ...
            while (true)
            {
                var cell = this.cells.FirstOrDefault(cell => cell.Count == 1 || cell.Count == -1);
                if (cell == null)
                {
                ...
                if (cell.Count == 1 && cell.HashKeySum == KeyHash(cell.KeySum))
                {
                    outputList.Add((cell.KeySum, cell.ValueSum));
                    this.Delete(cell.KeySum, cell.ValueSum);
                }
                else if (cell.Count == -1 && -cell.HashKeySum == KeyHash(-cell.KeySum)) // extaneous delete
                {
                    //outputList.Add((-cell.KeySum, -cell.ValueSum));
                    //this.Insert(-cell.KeySum, -cell.ValueSum);
                }
            }
